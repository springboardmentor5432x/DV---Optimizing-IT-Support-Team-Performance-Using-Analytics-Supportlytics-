...
# CONFIG
# -----------------------------
DATA_PATH = "customer_support_tickets.csv"   # change if your filename differs
OUT_CLEAN_DIR = "outputs/cleaned"
OUT_VIS_DIR = "outputs/visuals"
DOCS_DIR = "docs"
...
# -----------------------------
# 2) FIND CANDIDATE COLUMNS (robust)
# -----------------------------
...
col_type = find_col(["type", "ticket_type", "issue_type"])
col_priority = find_col(["priority", "priority_level", "urgency"])
col_category = find_col(["category", "issue_category", "topic"])
col_created = find_col(["created_date", "created at", "created", "date_created", "open_date", "opened"])
col_resolved = find_col(["resolved_date", "resolved at", "resolved", "closed_date", "close_date", "closed"])
...
# (handles common labels; unknown -> NaN)
priority_map = {
    "critical": 4,
    "high": 3,
    "medium": 2,
...
if col_priority:
    df_clean["Priority_Score"] = (
        df_clean[col_priority].astype(str).str.lower().map(priority_map)
    )
else:
...
# -----------------------------
clean_path = os.path.join(OUT_CLEAN_DIR, "cleaned_tickets.csv")
...
# Feature summary
fs_path = os.path.join(DOCS_DIR, "feature_summary.md")
